...
func TestAccessPostgres(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			_, _, err := testCtx.createUserAndRole(ctx, test.user, test.role, test.allowDbUsers, test.allowDbNames)
...
func TestAccessMySQL(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			// Create user/role with the requested permissions.
			_, _, err := testCtx.createUserAndRole(ctx, test.user, test.role, test.allowDbUsers, []string{types.Wildcard})
			require.NoError(t, err)
...
func TestAccessDisabled(t *testing.T) {
	...
	// Create user/role with the requested permissions.
	_, _, err := testCtx.createUserAndRole(ctx, userName, roleName, []string{types.Wildcard}, []string{types.Wildcard})
	require.NoError(t, err)
...
func (c *testContext) createUserAndRole(ctx context.Context, userName, roleName string, dbUsers, dbNames []string) (types.User, types.Role, error) {
...
func setupTestContext(ctx context.Context, t *testing.T, withDatabases ...withDatabaseOption) *testContext {
	...
	for _, withDatabase := range withDatabases {
		databaseServers = append(databaseServers,
			withDatabase(t, ctx, testCtx))
	}
