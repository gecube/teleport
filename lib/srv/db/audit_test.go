...
func TestAuditPostgres(t *testing.T) {
	...
	_, _, err := testCtx.createUserAndRole(ctx, "alice", "admin", []string{"postgres"}, []string{"postgres"})
...
func TestAuditMySQL(t *testing.T) {
	...
	_, _, err := testCtx.createUserAndRole(ctx, "alice", "admin", []string{"root"}, []string{types.Wildcard})
	require.NoError(t, err)
