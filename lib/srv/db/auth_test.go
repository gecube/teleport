...
func TestAuthTokens(t *testing.T) {
	...
	_, _, err := testCtx.createUserAndRole(ctx, "alice", "admin", []string{types.Wildcard}, []string{types.Wildcard})
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			default:
				t.Fatalf("unrecognized database protocol in test: %v", test)
			}
		})
	}
}
...
func (a *testAuth) GetTLSConfig(ctx context.Context, sessionCtx *common.Session) (*tls.Config, error) {
	return a.Auth.GetTLSConfig(ctx, sessionCtx)
}

// GetAuthPreference returns the cluster authentication config.
func (a *testAuth) GetAuthPreference() (services.AuthPreference, error) {
	return a.Auth.GetAuthPreference()
}
